// Adapted from React Bits' MagicBento. Unlike the original demo, this
// version takes an arbitrary `cards` array as a prop (title, value,
// description, span, icon) instead of six hardcoded cards, and reads
// colors from TrafficIQ's theme tokens / indigo-violet-cyan palette
// instead of the original purple. Requires gsap (npm install gsap).
...
const DEFAULT_GLOW_COLOR = "129, 140, 248"; // indigo-300 (#818CF8), matches hero accent
const MOBILE_BREAKPOINT = 768;

const createParticleElement = (x, y, color) => {
  const el = document.createElement("div");
  el.className = "mbg-particle";
  el.style.cssText = `
    position: absolute;
    width: 4px;
    height: 4px;
...
function ParticleCard({
  ...
  return (
    <div ref={cardRef} className={`mbg-particle-container ${className}`} style={{ ...style, position: "relative", overflow: "hidden" }}>
      {children}
    </div>
  );
}
...
function GlobalSpotlight({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) {
  ...
  useEffect(() => {
    ...
    spotlight.className = "mbg-global-spotlight";
    ...
    const handleMouseMove = (e) => {
      ...
      const section = gridRef.current.closest(".mbg-section");
 ...
 *   label?: string,
 *   title: string,
 *   value?: string,        // big stat, e.g. "1,284 veh/hr"
 *   description?: string,
 *   icon?: LucideIcon,
 *   colSpan?: 1 | 2,
 *   rowSpan?: 1 | 2,
 *   accent?: string,       // "r, g, b" to override glowColor per-card
 *   content?: ReactNode,   // custom body (chart, list, etc.) instead of value/description
 * }>
 */
export default function MagicBentoGrid({
  ...
  return (
    <>
      ...
      <div className="mbg-grid mbg-section" ref={gridRef}>
        {cards.map((card) => {
          const Icon = card.icon;
          const cardGlow = card.accent ?? glowColor;
          ...
          const body = (
            <>
              <div className="magic-bento-card__header">
                ...
                {Icon && <Icon size={16} className="magic-bento-card__icon" strokeWidth={1.75} />}
              </div>
          ...
          return enableStars ? (
            ...
            <div key={card.id} className={baseClassName} style={gridStyle}>
              {body}
            </div>
          );
        })}
